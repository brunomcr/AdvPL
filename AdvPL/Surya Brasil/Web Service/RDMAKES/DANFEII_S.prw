...
User Function PrtNfSefS(cIdEnt, cVal1		, cVal2,	oDanfe,; //surya
...
//oDanfe:Say(10,10,'Teste', ,1000, CLR_HRED)
// ----------------------------------------------
...
Else     
	if alltrim(funname())!=alltrim("RPC")//suryabrasil
		if alltrim(funname())!=alltrim("WFPREPENV")
...
If lPergunte
	...
	DANFEProc(@oDanfe,, cIDEnt, Nil, Nil, @lExistNFe, lIsLoja)	        // Suryabrasil
//	oDanfe:Preview()//Visualiza antes de imprimi    
EndIf

IF alltrim(funname())!=alltrim("RPC")      
		IF alltrim(funname())!=alltrim("WFPREPENV")
			IF lExistNFe
...
Local cUnitGrp		:= ""
Local cFilGrp		:= ""
Local lImpBol		:= SuperGetMV("SB_IMPBOL",.F.,.F.)
Local lPontilhado	:= .F.
