...
Local aItensAux     := {}
Local aArray		:= {}
Local aArrBol		:= {}     //surya Boleto
Local lImpBol		:= SuperGetMV("SB_IMPBOL",.F.,.F.)//Surya
Default cDtHrRecCab := ""
Default dDtReceb    := CToD("")
Private aInfNf  	:= {}
Private cEmpresa	:= cEmpAnt //Surya
...
//Surya ==>  
if cEmpresa == FC //
	AADD(aArrBol,xFilial("SE1"))//[1]
	AADD(aArrBol,SF2->F2_CLIENTE)//[2]
	AADD(aArrBol,SF2->F2_LOJA)//[3]
	AADD(aArrBol,SF2->F2_SERIE)//[4]
	AADD(aArrBol,SF2->F2_DOC)//[5]
	AADD(aArrBol,Posicione("SC5",1,xFilial("SC5")+SD2->D2_PEDIDO,"C5_XFORMA"))//[6]
	AADD(aArrBol,Posicione("SA1",1,xFilial("SA1")+SF2->F2_CLIENTE+SF2->F2_LOJA,"A1_XBOLENT"))//[7]
Endif	
//<==Surya
...
If MV_PAR05==1 .And. MV_PAR01 <> MV_PAR02 .And. (--nFolha)%2<>0
	...
	oDanfe:EndPage()
EndIf
    //impressao boleto surya
If cEmpresa == FC
	IF  lImpBol .And. aArrBol[6] == "1000" .And. aArrBol[7] <> "1"
		ImpBolX(aArrBol[1],aArrBol[2],aArrBol[3],aArrBol[4],aArrBol[5],oDanfe) 
	ENDIF
Endif
Return(.T.)
...
static Function ImpBolX(c_Filial,c_Cliente,c_Loja,c_Serie,c_Nfiscal,oDanfe)  
	U_ImpBolDF(c_Filial,c_Cliente,c_Loja,c_Serie,c_Nfiscal,oDanfe)   
//oDanfe:Say(0001,0100, c_Nfiscal,oFont08:oFont)      
return
User Function ImpBolDF(c_Filial,c_Cliente,c_Loja,c_Serie,c_Nfiscal,oDanfe)   
local aParam		:= {}
local cparcFirst	:= ""
local cparcLast		:= ""  
local lDanfe		:= .T. 
local cimpBol		:= ''
lOCAL c_ChvA6	:= SuperGetMV("SB_CHVA6BO",.t.,'')
Local cBanco	:= subs(c_ChvA6,17,3)
dbSelectArea("SE1")
SE1->(dbSetOrder(2))
If SE1->(dbSeek(c_Filial+c_Cliente+c_Loja+c_Serie+c_Nfiscal))
	cparcFirst := SE1->E1_PARCELA
	AADD(aParam, SE1->E1_PREFIXO)
	AADD(aParam, SE1->E1_NUM)
	AADD(aParam, SE1->E1_NUM)
	AADD(aParam, cparcFirst)
	cimpBol :=  SE1->E1_XIMPBOL
	While !SE1->(Eof()) .And. xFilial("SE1") == SE1->E1_FILIAL .And.;
		...
		cparcLast	:= SE1->E1_PARCELA
		SE1->(DbSkip ())
	EndDo	
	AADD(aParam, cparcLast) 
	AADD(aParam, cimpBol)
ENDIF 
if len(aParam) > 0	.And. cimpBol <> '1'
	Do Case
		Case cBanco == "237"
			U_BOLETBRA(lDanfe, aParam,oDanfe)  
		Case cBanco	== "033"
			U_BOLETSAN(lDanfe, aParam,oDanfe)
	EndCase
endif
Return		
