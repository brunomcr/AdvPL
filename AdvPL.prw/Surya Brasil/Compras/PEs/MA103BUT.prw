...
User Function MA103BUT()
	...
	AADD(aButton, {"", {|| U_SSCOM001()}, "Preenche Tp Oper"})
...
User Function SSCOM001()
	...
	Local aPerg		:= {}
	Local aRet		:= {}
	Local cCodTES 	:= Space(2)

	//Adiciona as informacoes necessarias no Array aPerg, para montagem de tela atraves da funcao Parambox
	AADD( aPerg ,{1,"Tipo de Operacao:", cCodTES, "@!", "StaticCall(SSCOM001, VdFillTES)", "SFM", '.T.', 40, .T.}) 
...
Static Function VdFillTES()
	...
	If (AllTrim(Mv_Par01) == "")
		...
		MsgAlert("Favor selecionar um Tipo de Operacao, para correta classificação do Documento de Entrada.", "Aviso")
...
Static Function ProcTES(cCodTES)
	...
	Local nLhAtual	:= 0
	Local nPosTES 	:= aScan(aHeader,{|x| Alltrim(x[2]) == "D1_OPER"})
	Local bValid    := {|| M->D1_OPER := cCodTES, CheckSx3("D1_OPER")}
